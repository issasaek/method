...
信念体系の最適化の第二段階は，第一段階である論理的還元（規格化）を通過した信念体系$\mathfrak{B}'$を，その存在論の全ての要素を供給する理論$\mathrm{ZFC}$を受容している信念体系$\mathfrak{B}''$へと改造する工程である．$\mathrm{ZFC}$は\fndp{9}{9}で提示した標準的な集合論の体系だ．
...
\begin{enumerate}[label=(\arabic*)]
    ...
    \item \kagi{$ (x)(Fx\case{1}{0}{1}x\subseteq\alpha) $}の\kagi{$ \alpha $}に$r$を，\kagi{$ Fx $}に$q$を代入した結果は，$t$のメンバーである\footnote{
        \kagi{$ (x)(Fx\case{1}{0}{1}x\subseteq\alpha) $}への代入結果は，$p$が属する理論$t'\subseteq t$と$u$を架橋する法則と言える．架橋法則により，\kagi{$ (\exists x)Fx $}と\kagi{$ (\exists x)(x\subseteq\alpha\con{1}Fx) $}の各代入結果から作られた双条件法が，$t$のメンバーになる（$t$で等値）．
    }．
    \item \kagi{$ (\exists x)(x = \alpha) $}の\kagi{$ \alpha $}に$r$を代入した結果は，$u$のメンバーである．
\end{enumerate}

今，$ l = \mathfrak{L}\con{1}t = \mathfrak{T} $として，$\mathfrak{T}$を標準的な科学とテクノロジーに関連する理論を実質的に全て含むような包括的な$\mathfrak{L}$の理論としよう．\fndpp{18}{18--20}においては，真理集合$\mathrm{T}^{(\breve{\epsilon}\fap 0)}\cap\mathfrak{L}$の部分クラスとして$\mathfrak{T}$が想定されている．また，$\mathfrak{T}$には$\mathrm{ZFC}$集合論から置換公理を除いた理論ZCが含まれ\footnote{正確には，原初的言語ではなく$\mathfrak{L}$に合わせてZCを再解釈したものが$\mathfrak{T}$に含まれる．}，$\mathfrak{T}$の存在論$s$の任意のメンバーがZCによって供給される（と想定される）．例えば$q$が「$x$はマイナスの電荷を持つ素粒子である」に相当する$\mathfrak{L}$の文とすると，\kagi{$ (x)(Fx\case{1}{0}{1}x\subseteq\timex{\mathbb{R}}{4}) $}の代入例が$\mathfrak{T}$で証明可能である（と想定される）．そして，\kagi{$ (\exists x)(x = \timex{\mathbb{R}}{4}) $}はZCで証明可能である．

次に，$ l = \mathfrak{L}\con{1}t = \mathfrak{B}' $としよう．第一段階を通過した信念体系$ \mathfrak{B}' $は，理想的には$\mathfrak{L}$の文のクラスであり，また理論$\mathfrak{T}$の部分クラスを含んでいる．例えば，$ \orp{a,b}\in x\bkg{\epsilon}y $であることを記述する文が$ \mathfrak{B}' $のメンバーであるとき，
\[
...
\end{align}
に相当する文（ZCのモデルの対象領域の存在を肯定する）が含まれ得る．また$\mathfrak{B}'$には，解釈空間に相対化された因果の概念だけでなく，それに基づく規制の概念も取り込まれている．すると規制類型の階層構造によって（\fndp{43}{43}），次の型式の\kagi{$ \gamma $}に$2$より大きい数を指示する抽象体を代入した結果も，$\mathfrak{B}'$に含まれ得る．
\begin{align}
    \text{
...
(1)(2)では\kagi{$ \union{(\mathcal{W}\img\mathbb{N})}\subseteq \alpha $}の代入結果がZCのメンバーになり，(3)では\kagi{$ \mathcal{W}\fap 3\subseteq \alpha $}の代入結果がZCのメンバーになるような抽象体だろう．すると，
\[
   \text{
...
\noindent 標準的な科学とテクノロジーが要求する数学は，\fndp{4}{4}の原初的言語に還元可能であり，その存在論は上記からA \ref{axim:置換}を除いた体系ZCによって供給できる．他方，物理理論その他の科学理論は原初的言語に還元可能ではないが，$\mathfrak{T}$の部分クラスであり，$\mathfrak{T}$の存在論はZCで供給可能である（\ref{sssec:物理的対象}）．さらに因果と規制の概念は，解釈空間に相対化する形式ならば，原初的言語に還元可能である．しかし，それが要求する（規制の概念の使用が想定される工学的理論の）存在論はZCでは供給できない．本稿で$\mathrm{ZFC}$が要請されたのはそのためだ．もっとも，$\mathrm{ZFC}$によって不可避的に，環境の予測と制御にとって必要な範囲を遥かに超えて，著しく存在論は拡張される．\ref{sssec:累積階層}でその全体像が確認される．
...
また，型式\kagi{$ (x)(Fx\case{1}{0}{1}x\subseteq \alpha) $}の\kagi{$ Fx $}に物理的対象を特徴づける文脈を，\kagi{$ \alpha $}に\kagi{$ \alpha\in\univ $}が$\mathrm{ZFC}$で証明可能なクラス抽象体を代入した結果のあるものが，$ \mathrm{T}^{(\breve{\epsilon}\fap\Lambda)} $のメンバーであるとみなされる（架橋法則）．
...
\]
を，\kagi{$ \alpha $}に\kagi{$ \timex{\mathbb{R}}{4}\times\timex{\mathbb{R}}{4} $}を代入した結果．なお\kagi{$ Gat $}は，物理的対象$ a $が時間$ t $に一定の状態にあることを記述する文脈である．
...
1つのモデルが特定されている場合，そこから代替モデルを作るのは極めて容易だ．解釈空間の開始点$\breve{\epsilon}\fap 0 $について考えよう．それは$\mathfrak{L}$における標準的モデルであり（\fndp{19}{19}），包括理論$\mathfrak{T}$のモデルであると想定されている（$ \mathfrak{T}\subseteq \mathrm{T}^{(\breve{\epsilon}\fap 0)} $）．
...
しかし，ここで何かを受容したり選択する主体はもはや人間ではない．$\union{(\mathrm{W}\img\univ)}$の中に人間であるような固有の対象はないからだ．あなたが普段それに自己同一化しているものは，選択する主体の仮想世界におけるアバターなのだ．
選択する主体は自由に解釈空間$e$を選択してビジョンを生成する．その後単純に別の解釈空間$ e' $を再選択してビジョンを再創造するルートの他に，おそらく，$e$のビジョンの一部であるアバターを通じて，$ \arg e = \arg e' $であるような$e'$を再選択するルートが作られている．ただし，アバターを通じての再創造は，単にアバターが持つ信念体系に適合したビジョンを自動的に生成するというものだ．あなたが特定のモデルに固執することによって現実が固定されるというのは，このプロセスを意味している．もし世界が不愉快なものであるなら，それは信念体系の最適化が不足していることの現れだ．したがって，メソッドを適用して信念体系を最適化すれば，不愉快な世界は消え去る．しかし，それだけではない．メソッドの要点は，言語を規格化して存在をクラスに還元し，物理的世界を相対化することだ．それが信念体系に浸透していくにつれて，創造のプロセスは自覚的なものに変わっていく．それは本来持っていた自由を取り戻し，最終的に選択する主体へと回帰するプロセスなのだ．
